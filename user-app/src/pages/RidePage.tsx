...
export function RidePage() {
  ...
  return (
    <div className="page-stack">
      <section className="section-heading visual-heading depth-surface" {...headingDepth}>
        <div>
          <h1>{t("ride.title")}</h1>
          <p>{t("ride.subtitle")}</p>
        </div>
        <img src="/assets/images/ill-order-a-car.svg" alt="" aria-hidden="true" />
      </section>
      ...
      {accessIssue ? (
        ...
        <Card as="form" className="ride-form" onSubmit={createRequest}>
          ...
          <Field label={t("ride.preferredTime")} type="datetime-local" value={preferredTime} onChange={(event) => setPreferredTime(event.target.value)} required />
          <Field
          ...
          <Card as="article" tone="strong" className="record-card" key={request.request_id}>
            ...
            {!accessIssue && matches[request.request_id]?.map((match) => (
              <div className="match-row" key={match.match_id}>
                <span>{t("ride.tripNumber", { id: match.trip_id })}</span>
                <strong>{Math.round(match.match_score)}%</strong>
                <Button variant="secondary" type="button" onClick={() => confirm(match.match_id)}>
                  {t("common.confirm")}
                </Button>
              </div>
            ))}
